//
//  CGPhotoPreviewViewController.swift
...
class CGPhotoPreviewViewController: CGBaseViewController, UICollectionViewDelegate, UICollectionViewDataSource ,UICollectionViewDelegateFlowLayout{
    ...
    //  数据源(预览选择时用)
    var photoData = CGPhotoDataSource()
    //  浏览数据源
    ...
    private var willDisplayCellAndIndex: (cell: CGPreviewCollectionViewCell, indexPath: IndexPath)?
    ...
    private lazy var thumbnailCollectionView: UICollectionView = {
        ...
        //  collectionView
        let collectionView = UICollectionView(frame: CGRect(x: 0, y: CGScreenHeight-CGHomeBarHeight-44 - 81, width: CGScreenWidth, height: 80), collectionViewLayout: flowLayout)
    ...
    private lazy var bottomView = CGAlbumBottomView(type: .noPreview)
    ...
    override func didMove(toParentViewController parent: UIViewController?) {
        if parent == nil {
            for id in self.requestIDs {
                CGCachingImageManager.default().cancelImageRequest(id)
            }
        }
    }
        ...
        if let index = self.photoData.assetArray.index(of: self.previewPhotoArray[currentIndex]) {
            ...
            if let selectIndex = self.photoData.seletedAssetArray.index(of: self.previewPhotoArray[currentIndex]){
                if selectStyle == .number{
                    image = UIImage.CGCreateImageWithView(view: CGPhotoNavigationViewController.CGGetSelectNuberView(index: "\(selectIndex + 1)"))
                }else{
                    image = CGSelectSkinImage
                }
            }
            self.setRightImageButton(normalImage: UIImage.CGImageFromeBundle(named: "album_select_gray.png"), selectedImage: image, isSelected: self.photoData.divideArray[index])
        }
    ...
    private func setPreviewImage(cell: CGPreviewCollectionViewCell, asset: PHAsset) {
        let pixelScale = CGFloat(asset.pixelWidth)/CGFloat(asset.pixelHeight)
        let id = CGCachingImageManager.default().requestPreviewImage(for: asset, progressHandler: { (progress: Double, error: Error?, pointer: UnsafeMutablePointer<ObjCBool>, dictionry: Dictionary?) in
            //下载进度
            DispatchQueue.main.async {
                let progressView = CGProgressView.showCGProgressView(in: cell.contentView, frame: CGRect(x: cell.frame.width-20-12, y: cell.frame.midY+(cell.frame.width/pixelScale-20)/2-12, width: 20, height: 20))
    ...
    override func rightButtonClick(button: UIButton) {
        ...
        if let index = self.photoData.assetArray.index(of: self.previewPhotoArray[currentIndex]) {
            ...
            if selectIndex != nil{
                self.rightButton.asyncSetImage(UIImage.CGCreateImageWithView(view: CGPhotoNavigationViewController.CGGetSelectNuberView(index: "\(selectIndex! + 1)")), for: .selected)
            }
    ...
    func thumbnailCollectionViewCellToggeleSelect(indexPath:IndexPath){
        ...
        cell?.contentView.layer.borderColor = CGPhotoAlbumSkinColor.cgColor
        ...
        if collectionView == thumbnailCollectionView {
            ...
            if isFristLoadCell == false{
                if let selectIndex = self.photoData.seletedAssetArray.index(of: self.previewPhotoArray[currentIndex]),selectIndex == indexPath.row{
                    ...
                    smallCell.contentView.layer.borderColor = CGPhotoAlbumSkinColor.cgColor
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if  collectionView == thumbnailCollectionView {
            ...
            let id = CGCachingImageManager.default().requestThumbnailImage(for: asset) { (image: UIImage?, dictionry: Dictionary?) in
                (cell as! CGPreviewCollectionViewCell).photoImage = image ?? UIImage()
            }
            self.requestIDs.append(id)
            
            self.willDisplayCellAndIndex = (cell as! CGPreviewCollectionViewCell, indexPath)
            if indexPath.row == self.currentIndex && self.isFirstCell {
                self.isFirstCell = false
                self.setPreviewImage(cell: cell as! CGPreviewCollectionViewCell, asset: asset)
            }
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if  collectionView == thumbnailCollectionView {
            ...
            return CGSize(width: CGScreenWidth+10, height: CGScreenHeight)
        }
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        if scrollView == photoCollectionView{
            if scrollView.contentOffset.x != self.scrollDistance {
                let currentCell = self.photoCollectionView.cellForItem(at: IndexPath(item: self.currentIndex, section: 0)) as! CGPreviewCollectionViewCell
