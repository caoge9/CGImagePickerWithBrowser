//
//  CGPhotoClipViewController.swift
...
class CGPhotoClipViewController: CGBaseViewController, UIScrollViewDelegate {
    
    var clipBounds: CGSize = CGSize(width: CGScreenWidth, height: CGScreenWidth) {
        didSet {
            self.photoImageScrollView.frame = CGRect(origin: CGPoint(x: (CGScreenWidth-clipBounds.width)/2, y: (CGScreenHeight-clipBounds.height)/2), size: clipBounds)
        }
    }
    ...
    private lazy var photoImageScrollView: UIScrollView = {
        ...
        var frame = CGRect(origin: CGPoint(x: 0, y: (CGScreenHeight-CGScreenWidth)/2), size: self.clipBounds)
    ...
    private lazy var maskView: CGPhotoClipMaskView = {
        let view = CGPhotoClipMaskView(frame: self.view.frame, clipBounds: self.clipBounds)
        return view
    }()
    ...
    var photoImage: UIImage? {
        didSet {
            ...
            setContent = true
            let imageHeight = CGScreenWidth*size.height/size.width
            self.photoImageView.frame = CGRect(x: 0, y: 0, width: CGScreenWidth, height: imageHeight)
            ...
            if CGScreenWidth > clipBounds.width {
                self.photoImageScrollView.contentOffset.x = (CGScreenWidth-clipBounds.width)/2
            } else {
    ...
    override public func viewDidLoad() {
        ...
        if #available(iOS 11.0, *) {
            self.photoImageScrollView.contentInsetAdjustmentBehavior = .never
        } else {
    ...
    func scrollViewDidZoom(_ scrollView: UIScrollView) {
        var xcenter = scrollView.center.x-(CGScreenWidth-clipBounds.width)/2 , ycenter = scrollView.center.y-(CGScreenHeight-clipBounds.height)/2
...
fileprivate class CGPhotoClipMaskView: UIView {
    ...
    convenience override init(frame: CGRect) {
        self.init(frame: frame, clipBounds: CGSize(width: CGScreenWidth, height: CGScreenWidth))
    }
    ...
    init(frame: CGRect, clipBounds: CGSize) {
        ...
        path.append(UIBezierPath(rect: CGRect(origin: CGPoint(x: (CGScreenWidth-clipBounds.width)/2, y: (CGScreenHeight-clipBounds.height)/2), size: clipBounds)).reversing())
