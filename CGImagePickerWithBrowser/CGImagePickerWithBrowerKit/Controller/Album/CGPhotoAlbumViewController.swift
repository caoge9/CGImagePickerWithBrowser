//
//  CGPhotoAlbumViewController.swift
//  CGPhotoAlbumViewController
//
...
class CGPhotoAlbumViewController: CGBaseViewController, PHPhotoLibraryChangeObserver, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    var type: CGPhotoAlbumType = .selectPhoto
    
    // 剪裁大小
    var clipBounds: CGSize = CGSize(width: CGScreenWidth, height: CGScreenWidth)
    
    weak var photoAlbumDelegate: CGPhotoAlbumProtocol?
    
    private let cellIdentifier = "PhotoCollectionCell"
    private lazy var photoCollectionView: UICollectionView = {
        // 竖屏时每行显示4张图片
        let shape: CGFloat = 5
        let cellWidth: CGFloat = (CGScreenWidth - 5 * shape) / 4
        let flowLayout = UICollectionViewFlowLayout()
        flowLayout.sectionInset = UIEdgeInsetsMake(CGNavigationTotalHeight, shape, self.type == .selectPhoto ? 44+CGHomeBarHeight:0, shape)
        flowLayout.itemSize = CGSize(width: cellWidth, height: cellWidth)
        flowLayout.minimumLineSpacing = shape
        flowLayout.minimumInteritemSpacing = shape
        //  collectionView
        let collectionView = UICollectionView(frame: CGRect(x: 0, y: 0, width: CGScreenWidth, height: CGScreenHeight), collectionViewLayout: flowLayout)
        collectionView.backgroundColor = UIColor.white
        collectionView.scrollIndicatorInsets = UIEdgeInsetsMake(CGNavigationTotalHeight, 0, 44+CGHomeBarHeight, 0)
        //  添加协议方法
        collectionView.delegate = self
        collectionView.dataSource = self
        //  设置 cell
        collectionView.register(CGPhotoCollectionViewCell.self, forCellWithReuseIdentifier: self.cellIdentifier)
        return collectionView
    }()
    
    private var bottomView = CGAlbumBottomView()
    private lazy var loadingView: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: CGNavigationTotalHeight, width: CGScreenWidth, height: CGScreenHeight-CGNavigationTotalHeight))
        ...
        loadingBackView.image = UIImage.CGCreateImageWithColor(color: UIColor(white: 0, alpha: 0.8), size: CGSize(width: 108, height: 108))?.CGSetRoundedCorner(radius: 6)
    ...
    //  数据源
    private var photoData = CGPhotoDataSource()
    ...
    private func gotoPreviewViewController(previewArray: [PHAsset], currentIndex: Int,souceType:Int) {
        let previewVC = CGPhotoPreviewViewController()
    ...
    private func gotoClipViewController(photoImage: UIImage) {
        let clipVC = CGPhotoClipViewController()
        clipVC.clipBounds = self.clipBounds
        clipVC.photoImage = photoImage
        clipVC.sureClicked = { [unowned self] (clipPhoto: UIImage?) in
            if self.photoAlbumDelegate != nil, self.photoAlbumDelegate!.responds(to: #selector(CGPhotoAlbumProtocol.photoAlbum(clipPhoto:))) {
    ...
    private func selectPhotoCell(cell: CGPhotoCollectionViewCell, index: Int) {
    ...
    private func selectSuccess(fromeView: UIView, selectAssetArray: [PHAsset]) {
        ...
        var selectPhotos: [CGPhotoModel] = Array(repeating: CGPhotoModel(), count: selectAssetArray.count)
        let group = DispatchGroup()
        for i in 0 ..< selectAssetArray.count {
            let asset = selectAssetArray[i]
            group.enter()
            let photoModel = CGPhotoModel()
            _ = CGCachingImageManager.default().requestThumbnailImage(for: asset, resultHandler: { (image: UIImage?, dictionry: Dictionary?) in
                photoModel.thumbnailImage = image
            })
            _ = CGCachingImageManager.default().requestPreviewImage(for: asset, progressHandler: nil, resultHandler: { (image: UIImage?, dictionry: Dictionary?) in
        ...
        group.notify(queue: DispatchQueue.main, execute: {
            ...
            if self.photoAlbumDelegate != nil {
                if self.photoAlbumDelegate!.responds(to: #selector(CGPhotoAlbumProtocol.photoAlbum(selectPhotoAssets:))){
                    self.photoAlbumDelegate?.photoAlbum!(selectPhotoAssets: selectAssetArray)
                }
                if self.photoAlbumDelegate!.responds(to: #selector(CGPhotoAlbumProtocol.photoAlbum(selectPhotos:))) {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellIdentifier, for: indexPath) as? CGPhotoCollectionViewCell, self.photoData.assetArray.count > indexPath.row else {return CGPhotoCollectionViewCell()}
        ...
        if type == .selectPhoto {
            ...
            if selectStyle == .number {
                if let Index = photoData.seletedAssetArray.index(of: asset) {
                    ...
                    cell.selectButton.asyncSetImage(UIImage.CGCreateImageWithView(view: CGPhotoNavigationViewController.CGGetSelectNuberView(index: "\(Index + 1)")), for: .selected)
                }else{
...
// 相册底部view
class CGAlbumBottomView: UIView {
    ...
    private lazy var sureButton: UIButton = {
        let button = UIButton(frame: CGRect(x: CGScreenWidth-12-64, y: 6, width: 64, height: 32))
        button.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        button.setTitle("完成", for: .normal)
        button.setBackgroundImage(UIImage.CGCreateImageWithColor(color: CGPhotoAlbumSkinColor, size: CGSize(width: 64, height: 32))?.CGSetRoundedCorner(radius: 4), for: .normal)
        button.setBackgroundImage(UIImage.CGCreateImageWithColor(color: CGPhotoAlbumSkinColor.withAlphaComponent(0.5), size: CGSize(width: 64, height: 32))?.CGSetRoundedCorner(radius: 4), for: .disabled)
        button.setTitleColor(UIColor(white: 0.5, alpha: 1), for: .disabled)
        button.setTitleColor(UIColor.white, for: .normal)
    ...
    enum CGAlbumBottomViewType {
        case normal, noPreview
    }
    
    convenience init() {
        self.init(frame: CGRect(x: 0, y: CGScreenHeight-CGHomeBarHeight-44, width: CGScreenWidth, height: 44+CGHomeBarHeight), type: .normal)
    }
    
    convenience init(type: CGAlbumBottomViewType) {
        self.init(frame: CGRect(x: 0, y: CGScreenHeight-CGHomeBarHeight-44, width: CGScreenWidth, height: 44+CGHomeBarHeight), type: type)
    }
    ...
    init(frame: CGRect, type: CGAlbumBottomViewType) {
