...
public class CGCachingImageManager: PHCachingImageManager {
    ...
    private static let shared = CGCachingImageManager()
    
    public override class func `default`() -> CGCachingImageManager {
        return shared
    }
    ...
    private func getThumbnailSize(originSize: CGSize) -> CGSize {
        let thumbnailWidth: CGFloat = (CGScreenWidth - 5 * 5) / 4 * UIScreen.main.scale
    ...
    func readCacheFromUrl(url: String) -> Data? {
        ...
        if data == nil {
            let path: String = CGCachingImageManager.default().getFullCachePathFromUrl(url: url)
    ...
    func getFullCachePathFromUrl(url: String) -> String {
        var chchePath = NSHomeDirectory().appending("/Library/Caches/CGImageCache")
        ...
        //进行字符串处理
        var newURL: String
        newURL = CGCachingImageManager.default().stringToString(str: url)
    ...
    public func removeAllCache(){
        ...
        let chchePath = NSHomeDirectory().appending("/Library/Caches/CGImageCache")
        let fileManager: FileManager = FileManager.default
        if fileManager.fileExists(atPath: chchePath) {
            do {
