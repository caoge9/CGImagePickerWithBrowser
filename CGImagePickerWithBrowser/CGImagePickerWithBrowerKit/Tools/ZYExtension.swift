...
extension UIImage {
    class func CGImageFromeBundle(named: String) -> UIImage? {
        return UIImage(named: named)
//        if let cacheImageData = CGCachingImageManager.default().getImageMemoryCache(key: named) {
...
//                CGCachingImageManager.default().setImageMemoryCache(key: named, data: imageData)
//            }
//            return image
//        }
//        return nil
    ...
    class func CGCreateImageWithColor(color: UIColor, size: CGSize) -> UIImage? {
    ...
    class func CGCreateImageWithView(view: UIView) -> UIImage? {
...
extension UIImage {
    func CGSetRoundedCorner(radius: CGFloat) -> UIImage? {
...
extension UIImageView {
    func asyncSetImage(_ image:UIImage?){
        DispatchQueue.global(qos: .userInteractive).async {
            let decodeImage = image?.CGDecodedImage()
    ...
    func setWebImage(url:String?, defaultImage:UIImage?, isCache:Bool, downloadSuccess: ((_ image: UIImage?) -> Void)?) {
        ...
        if isCache {
            var data: Data? = CGCachingImageManager.default().readCacheFromUrl(url: url!)
            if data != nil {
                CGImage = UIImage(data: data!)
                self.asyncSetImage(CGImage)
                if downloadSuccess != nil {
                    downloadSuccess!(CGImage)
                }
            }else{
                ...
                dispath.async(execute: { () -> Void in
                    do {
                        ...
                        if data != nil {
                            CGImage = UIImage(data: data!)
                            //写缓存
                            CGCachingImageManager.default().writeCacheToUrl(url: url!, data: data!)
